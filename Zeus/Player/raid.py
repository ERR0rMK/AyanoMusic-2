...
async def spam(e):
    ...
    if e.sender_id in SUDO_USERS:
        ...
        if len(Deadly) == 2:
            ...
            g = a.id
            if int(g) in Deadly:
                text = f"I can't raid on @AYANO_xD_bot's Owner"
    ...
    if e.sender_id in SUDO_USERS:
        ...
        if len(e.text) > 11:
            ...
            if int(user_id) in Deadly:
                text = f" can't raid on @AYANO_xD_bot's Owner."
                await e.reply(text, parse_mode=None, link_preview=None )
